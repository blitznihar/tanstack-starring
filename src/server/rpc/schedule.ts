...
/** Admin/parent schedule oversight for the demo child. */
export const managedPlans = createServerFn({ method: "GET" }).handler(async () => {
  const auth = await requireAuth();
  ...
  const [students, programs] = await Promise.all([
    usersRepo.list().then((users) => users.filter((u) => u.roles.includes("student") && u.active)),
    programsRepo.list(),
  ]);
  const programByKey = new Map(programs.map((p) => [p.key, p]));
  const plans: Array<PlanView & { studentName: string }> = [];

  for (const student of students) {
    if (!student._id) continue;
    const enrollments = await enrollmentsRepo.listForStudent(student._id);
    for (const e of enrollments.filter((enrollment) => enrollment.status === "active")) {
      if (!e._id) continue;
      const program = programByKey.get(e.programKey);
      const s = await getOrCreateSchedule(auth, e._id);
      plans.push({ studentName: student.displayName, ...toPlanView(e._id, e.programKey, program?.title ?? e.programKey, s) });
    }
  }

  return { displayName: auth.displayName, plans };
});
